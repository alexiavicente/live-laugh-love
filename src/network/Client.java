...
/**
 * network.Client is the main class that communicates with the server.TripleLServer.
 ...
 * any other messages that the server may of sent out. It depends on TripleLModel to
...
public class Client implements Runnable {
     ...
     * server.TripleLServer. It will create a new socket with the host and port that
     ...
     * messages being sent from the server.TripleLServer to the network.Client. If
     * it receives a message, it will send the text to the model.TripleLWorld
     */
    public void run() {
        ...
        while (true) {
            try {
                ...
                try {
                    String temp = ((Communication) this.inputStream.readObject()).getText();
                    Controller.getController().addInfo(new JSONObject(temp));
                } catch (IOException e) {
                    Controller.getController().addInfo("Disconnected from server");
                    break;
