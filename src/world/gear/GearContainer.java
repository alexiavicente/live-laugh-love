...
public class GearContainer extends Gear implements GearList {
     ...
     */
    public GearContainer(String name, String description, int maxSize, boolean canCarry) {
    ...
    public boolean giveGear(Movable movableToNotify, String gear, String otherName) {
        ...
        Gear item = this.getGear(gear);
        if (item == null) {
            if (movableToNotify != null)
                movableToNotify.sendToPlayer(gear + " is not in your inventory.");
            return false;
        }
        ...
        if (loc == null) {
            ...
            if (movableToNotify != null)
                movableToNotify.sendToPlayer(this.getName() + " location failure.");
            return false;
        }

        GearList other = null;
        if (World.getInstance().playerExists(otherName) && World.getInstance().playerIsLoggedOn(otherName)) {
            ...
            if (loc != null) {
                for (Gear roomItem : loc.listGear()) {
                    if (roomItem instanceof GearContainer && otherName.equals(roomItem.getName().toLowerCase())) {
        ...
        if (movableToNotify != null) {
            ...
            if (this.gearList.remove(item)) {
                ...
                movableToNotify.sendToPlayer(item.getName() + " has been removed from your inventory.");
                ...
                movableToNotify.sendToPlayer(this.getName()
                        + " remove failure: " + item.getName() + " from " + otherName + ".");
                System.out.println(this.getName() + " remove failure: " + item.getName() + " from " + otherName + ".");
            }
        } else {
    ...
    public JSONObject toJSONObject() {
        ...
        container.put("gears", gears);
        return container;
    }

    @Override
    public Gear cloneGear() {
        GearContainer clone = new GearContainer(getName(), getDescription(), maxSize, canCarry);
        clone.setPrice(getPrice());
        clone.setMaxGearCount(getMaxGearCount());
        for(Gear g: gearList)
            clone.addGear(g.cloneGear());
